...
📂 **GitHub:** [curiosity-matrix/Employee-Attrition-Predictor](https://github.com/curiosity-matrix/Employee-Attrition-Predictor)

---

## 🤔 What This Is

**AttritionIQ** is a full-stack HR analytics application that predicts employee attrition using a deep neural network trained on IBM HR Analytics data. Fill in an employee's profile — and the model tells you, with a probability score, whether that person is likely to leave.
...
| ⚖️ Data Balancing | SMOTE (imblearn) |
...
- ⚖️ **SMOTE** — Applied Synthetic Minority Oversampling to balance the training set before model training
- 🧠 **ANN Architecture** — `128 → 64 → 32 → 1 (sigmoid)` with Dropout layers (0.3, 0.3, 0.2) to prevent overfitting
- 🏋️ **Training** — Adam optimizer, binary cross-entropy loss, EarlyStopping with patience=10
...
⚠️ **Known limitation:** Class 1 recall is only 40% — the model misses some actual leavers. Lowering the decision threshold from 0.5 → 0.3 can improve this. AUC-ROC would give a better evaluation picture than accuracy alone.
...
- ✈️ **Frequent business travel** correlates with significantly higher attrition
...
- ⚖️ How to handle **class imbalance** in real datasets using SMOTE
...
- ☁️ Full backend deployment on Render / Railway
